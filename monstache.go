...
import (
	...
	"github.com/rwynn/monstache/v5/pkg/oplog"
	...
	if config.Mapping != nil {
		for _, m := range config.Mapping {
			if m.Namespace != "" && (m.Index != "" || m.Type != "") {
				...
				errorLog.Fatalln("Mappings must specify namespace and at least one of index and type")
			}
		}
	}
}
...
func (ic *indexClient) doDelete(op *gtm.Op) {
	...
	objectID, indexType, meta := opIDToString(op), ic.mapIndexType(op), &indexingMeta{}
	if objectID == "" {
